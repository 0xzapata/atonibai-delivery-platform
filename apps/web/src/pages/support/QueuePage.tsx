  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-xl font-extrabold tracking-tight sm:text-2xl">
            Ticket queue{' '}
            <span className="ml-1 rounded-full bg-stone-200 px-2 py-0.5 align-middle text-xs font-bold text-stone-600">
              {tickets.length}
            </span>
          </h1>
          <p className="mt-1 text-sm text-stone-500">
            Filter by status — threads poll every 6s once opened.
          </p>
        </div>
            ...
            {FILTERS.map((f) => (
              <button
                ...
                className={`rounded-full px-3 py-1.5 ${
                  filter === f.key ? 'bg-stone-900 text-white' : 'text-stone-600'
                }`}
              >
                {f.label}
              </button>
      ...
      {ticketsQuery.isError && (
        <ErrorState
          message="Ticket queue unavailable — is the API running at localhost:3001? (GET /api/tickets polls every 6s once reachable.)"
          onRetry={() => void ticketsQuery.refetch()}
        />
      )}

      {ticketsQuery.isSuccess && tickets.length === 0 && (
        <EmptyState message={`No ${filter === 'all' ? '' : `${filter} `}tickets right now.`} />
      )}
      ...
      {ticketsQuery.isSuccess && tickets.length > 0 && (
        <ul className="space-y-2">
          {tickets.map((t) => (
            <li key={t.id}>
              <Link
                to={`t/${encodeURIComponent(t.id)}`}
                className="card flex flex-wrap items-center gap-2 p-4 transition hover:-translate-y-0.5 hover:shadow-md"
              >
                <span className="text-sm font-extrabold">#{shortId(t.id)}</span>
                <TicketStatusBadge status={t.status} />
                <PriorityBadge priority={t.priority} />
                <span className="w-full truncate text-sm font-bold sm:w-auto sm:flex-1">
                  {t.subject}
                </span>
                <span className="text-xs font-semibold text-stone-400">
                  {t.order_id ? `order #${shortId(t.order_id)} · ` : ''}
                  {t.opener ? `${t.opener} · ` : ''}
                  {formatDateTime(t.created_at)}
                  {t.message_count !== null && t.message_count !== undefined
