...
function OrderContextCard({ orderId }: { orderId: string }) {
  const orderQuery = useQuery({
    ...
    queryFn: () => fetchOrderContext(orderId),
  ...
  if (orderQuery.isPending) {
    return (
      <div className="card animate-pulse p-4 text-sm font-semibold text-stone-500">
    ...
    return (
      <div className="card p-4">
        ...
        <p className="mt-1 text-xs font-semibold text-stone-500">
          Order context unavailable (GET /api/orders/:id) — actions below still work.
        </p>
      </div>
    );
  }
  ...
  const paymentStatus = str(o.payment_status ?? o.paymentStatus);
  const store = asRecord(o.store);
  const offer = asRecord(o.latest_offer);
  const items = Array.isArray(o.items) ? o.items : [];
  const riderName = str(offer.rider_name ?? offer.riderName);
  ...
  return (
    <div className="card space-y-2 p-4">
      ...
      <dl className="grid grid-cols-1 gap-1 text-xs font-semibold text-stone-600 sm:grid-cols-2">
        <div>
          <dt className="text-stone-400">Store</dt>
          <dd>{str(store.name) ?? '—'}</dd>
        </div>
        <div>
          <dt className="text-stone-400">Rider (latest offer)</dt>
          <dd>
            {riderName ?? '—'}
            {str(offer.status) ? ` · ${str(offer.status)}` : ''}
          </dd>
        </div>
        <div>
          <dt className="text-stone-400">Items</dt>
          <dd>{items.length > 0 ? `${items.length} line${items.length === 1 ? '' : 's'}` : '—'}</dd>
        </div>
        <div>
          <dt className="text-stone-400">Placed</dt>
          <dd>{formatDateTime(str(o.created_at ?? o.createdAt))}</dd>
        </div>
      </dl>
    </div>
  );
...
function ReassignModal({
  ...
  const ridersQuery = useQuery({
    queryKey: ['support', 'riders'],
    queryFn: fetchRidersForReassign,
    retry: false,
    refetchOnWindowFocus: false,
    staleTime: 30_000,
  });
  ...
  async function submit() {
    ...
    if (!target) {
      toast.push('Pick a rider or paste a rider ID', 'err');
      return;
    }
    setSending(true);
    try {
      ...
      toast.push(e instanceof Error ? e.message : 'Reassign failed', 'err');
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label={`Reassign order ${orderId}`}
      ...
      >
        <div>
          <h2 className="text-base font-extrabold">
            Reassign order #{shortId(orderId)}
          </h2>
          <p className="mt-0.5 text-xs text-stone-500">
            POST /api/ops/assign with the support persona.
          </p>
        </div>
        {ridersQuery.isSuccess && riders.length > 0 ? (
          ...
          <p className="rounded-xl bg-stone-100 px-3 py-2 text-xs font-semibold text-stone-600">
            {ridersQuery.isPending
              ...
              : 'Rider directory is operator-only — paste a rider ID below.'}
          </p>
        )}
        <label className="block text-sm font-bold">
          {riders.length > 0 ? 'Or rider ID (UUID)' : 'Rider ID (UUID)'}
          <input
            value={manualId}
            onChange={(e) => {
              setManualId(e.target.value);
              if (e.target.value.trim()) setRiderId('');
            }}
...
function ThreadInner({ ticketId }: { ticketId: string }) {
  ...
  async function sendReply() {
    ...
    if (!body) {
      toast.push('Type a reply first', 'err');
      return;
    }
    setSending(true);
    try {
      ...
      toast.push(e instanceof Error ? e.message : 'Reply failed', 'err');
    } finally {
      setSending(false);
    }
  }
  ...
  async function doResolve() {
    ...
    try {
      ...
      toast.push('Ticket resolved', 'ok');
      await refresh();
    } catch (e) {
      toast.push(e instanceof Error ? e.message : 'Resolve failed', 'err');
    } finally {
      setActing(null);
    }
  }
  ...
  async function doRefund() {
    if (!ticket?.order_id) {
      toast.push('Ticket has no linked order — refund unavailable', 'err');
      return;
    }
    setActing('refund');
    try {
      await refundTicket(ticketId);
      toast.push('Refund issued', 'ok');
      await refresh();
      await queryClient.invalidateQueries({
        queryKey: ['support', 'order', ticket.order_id],
      });
    } catch (e) {
      toast.push(e instanceof Error ? e.message : 'Refund failed', 'err');
    } finally {
      setActing(null);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      {detailQuery.isPending && (
        <div className="card animate-pulse p-6 text-sm text-stone-500">Loading thread…</div>
      )}

      {detailQuery.isError && (
        <ErrorState
          message="Couldn't load this ticket — is the API running at localhost:3001?"
          onRetry={() => void detailQuery.refetch()}
        />
      )}

      {detailQuery.isSuccess && !ticket && (
        <EmptyState message="Ticket not found — it may have been deleted." />
      )}
        ...
        <>
          <div className="card space-y-2 p-4">
            ...
            <p className="text-xs font-semibold text-stone-500">
              #{shortId(ticket.id)}
              {ticket.order_id ? ` · order #${shortId(ticket.order_id)}` : ' · no linked order'}
          ...
          <div className="card space-y-2 p-4">
            ...
            {messages.length === 0 && (
              <EmptyState message="No messages yet — send the first reply below." />
            )}
            ...
            <div className="flex flex-wrap gap-1.5">
              {CANNED.map((c) => (
                <button
                  ...
                  type="button"
                  onClick={() => setDraft(c)}
                  className="rounded-full bg-stone-100 px-3 py-1.5 text-xs font-bold text-stone-600 hover:bg-stone-200"
                >
                  {c}
                </button>
              ))}
          ...
          <div className="card flex flex-wrap gap-2 p-4">
            <button
              ...
              disabled={acting !== null || ticket.status.toLowerCase() === 'resolved'}
              className="rounded-full bg-stone-900 px-4 py-2 text-sm font-bold text-white disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={acting !== null || !ticket.order_id}
              title={ticket.order_id ? 'Refund the linked order' : 'No linked order'}
              className="rounded-full bg-amber-500 px-4 py-2 text-sm font-bold text-white disabled:opacity-50"
            >
            ...
            <button
              ...
              onClick={() => {
                if (!ticket.order_id) {
                  toast.push('Ticket has no linked order — reassign unavailable', 'err');
                  return;
                }
                setReassignOpen(true);
              }}
              disabled={!ticket.order_id}
              title={ticket.order_id ? 'Force-assign a rider' : 'No linked order'}
              className="rounded-full px-4 py-2 text-sm font-bold text-stone-700 ring-1 ring-stone-300 disabled:opacity-50"
            >
              Reassign rider
            </button>
          </div>
              ...
              onClose={(done) => {
                ...
                if (done) {
                  void queryClient.invalidateQueries({
                    queryKey: ['support', 'order', ticket.order_id as string],
                  });
                }
              }}
            />
