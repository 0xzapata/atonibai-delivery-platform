...
import { fetchActiveOrders, fetchAllOrders, orderBuyer, orderCreatedAt, orderItemsCount, orderTotal } from './storeApi';
import {
  EmptyState, ErrorState, GROUP_LABELS, PageHeader, StatusBadge,
  StoreShell, formatDateTime, formatPeso, groupForStatus,
} from './ui';
...
function OrderCard({ order }: { order: OrderSummary }) {
  return (
    <Link to={`/store/orders/${order.id}`} className="card block p-3 transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <p className="mt-1 text-xs font-semibold text-stone-400">{formatDateTime(orderCreatedAt(order))}</p>
    </Link>
  );
}
...
export default function OrdersPage() {
  ...
  const grouped = useMemo(() => {
    const base: Record<string, OrderSummary[]> = { new: [], preparing: [], ready: [], on_the_way: [], done: [] };
    for (const o of ordersQuery.data ?? []) base[groupForStatus(o.status)]?.push(o);
    return base;
  }, [ordersQuery.data]);
  ...
  return (
    <StoreShell>
      <PageHeader title="Orders" blurb="Kanban by kitchen stage. Cards open full detail with next-step actions."
        right={
          <div className="flex rounded-full border border-stone-200 bg-white p-1 text-xs font-bold">
            {SCOPES.map((s) => (
              <button key={s} type="button" onClick={() => setScope(s)}
                className={`rounded-full px-3 py-1.5 ${scope === s ? 'bg-stone-900 text-white' : 'text-stone-600'}`}>
        ...
        <>
            ...
            {COLUMNS.map((col) => (
              <section key={col.key} className="card flex flex-col p-3">
                <header className="flex items-baseline justify-between px-1 pb-2">
                  <h2 className="text-sm font-extrabold">{col.title}{' '}
                    <span className="ml-1 rounded-full bg-stone-100 px-2 py-0.5 text-xs font-bold text-stone-500">{grouped[col.key].length}</span>
                  </h2>
                </header>
                <p className="px-1 pb-2 text-xs font-semibold text-stone-400">{col.hint}</p>
                <div className="space-y-2">
                  {grouped[col.key].length === 0 ? (
                    <p className="rounded-xl bg-stone-50 px-3 py-4 text-center text-xs font-semibold text-stone-400">Empty</p>
                  ) : (
                    grouped[col.key].map((o) => <OrderCard key={String(o.id)} order={o} />)
                  )}
                </div>
              </section>
            ))}
          ...
          {scope === 'all' && doneCount > 0 ? (
            <section className="card p-4">
              <h2 className="text-sm font-extrabold">Done{' '}
                <span className="ml-1 rounded-full bg-stone-100 px-2 py-0.5 text-xs font-bold text-stone-500">{doneCount}</span>
              </h2>
              <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {grouped.done.map((o) => <OrderCard key={String(o.id)} order={o} />)}
              </div>
            </section>
          ) : null}
        </>
