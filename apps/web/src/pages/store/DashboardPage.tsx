...
function NewOrdersPanel() {
  ...
  const orders = (ordersQuery.data ?? []).filter(
    (o) => o.status === 'placed' || o.status === 'store_accepted',
  );
  ...
  return (
    <section className="card p-4 sm:p-5">
      ...
      <div className="mt-3 space-y-2">
        {ordersQuery.isPending ? (
          ...
          orders.map((o) => (
            <div
              key={String(o.id)}
              className="flex flex-col gap-2 rounded-2xl border border-stone-200 p-3 sm:flex-row sm:items-center sm:justify-between"
            >
              <div className="min-w-0">
                ...
                <p className="mt-1 truncate text-sm text-stone-600">
                  {orderBuyer(o)} · {orderItemsCount(o)} item(s) ·{' '}
                  {formatDateTime(orderCreatedAt(o))}
                </p>
              ...
              <div className="flex shrink-0 items-center gap-2">
                <button
                  ...
                  onClick={() =>
                    actionMutation.mutate({ id: o.id, action: 'accept' })
                  }
                ...
                <button
                  ...
                  disabled={actionMutation.isPending}
                  onClick={() =>
                    actionMutation.mutate({ id: o.id, action: 'reject' })
                  }
                  className="rounded-full border border-red-200 bg-red-50 px-4 py-1.5 text-xs font-bold text-red-700 disabled:opacity-50"
                >
                  Reject
                </button>
              </div>
      ...
      <Link
        to="/store/orders"
        className="mt-3 inline-block text-sm font-bold text-stone-700 hover:underline"
      >
        Open order kanban →
      </Link>
    </section>
...
export default function DashboardPage() {
  ...
  return (
    <StoreShell>
      <PageHeader
        title={selectedName ? `${selectedName} — Dashboard` : 'Store dashboard'}
      ...
      {statsQuery.isError ? (
        <ErrorState
          message="Stats unavailable — is the API running at localhost:3001?"
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <StatCard
            label="Today revenue"
            value={statsQuery.isPending ? '…' : formatPeso(stats.todayRevenue)}
            sub="Gross sales today (₱)"
          />
          <StatCard
            label="Active orders"
            value={statsQuery.isPending ? '…' : String(stats.activeCount)}
            sub="Placed → delivering"
          />
          <StatCard
            label="Delivered today"
            value={statsQuery.isPending ? '…' : String(stats.deliveredToday)}
            sub="Completed dropoffs"
          />
        </div>
      )}
      <NewOrdersPanel />
