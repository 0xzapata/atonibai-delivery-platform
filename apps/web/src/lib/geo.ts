...
export async function fetchRoute(fromLng: number, fromLat: number, toLng: number, toLat: number): Promise<Array<[number, number]> | null> {
  try {
    // The demo server can hang — cap the wait so the UI falls back to a straight line.
    const res = await fetch(`https://router.project-osrm.org/route/v1/driving/${fromLng},${fromLat};${toLng},${toLat}?overview=full&geometries=geojson`, { signal: AbortSignal.timeout(8_000) });
