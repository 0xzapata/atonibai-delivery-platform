...
// POC allowlist: demo clients run on localhost (Socket.IO registration below uses the same list).
...
// One fan-out (replaces 5 emit* helpers): order room (when orderId) + city.ops.
// Rooms ONLY — never io.emit globally (would leak every order's live GPS + tickets to all clients).
...
async function dispatchOrder(orderId: string): Promise<void> {
  try {
    // Retire time-expired 'offered' rows up front (in-memory expiry timers die with the process).
    await q(`UPDATE offers SET status = 'expired' WHERE order_id = $1 AND status = 'offered' AND expires_at IS NOT NULL AND expires_at <= now()`, [orderId]);
...
// On cancel / store-reject / support-refund: expire live offers, flip the rider back to 'online'.
// Re-sweep unassigned dispatchables when a rider comes online (debounced); expiry is a DB
// timestamp so a crash-safe sweep at boot + interval recovers rows a dead process left 'offered'.
async function releaseOrderAssignment(orderId: string): Promise<void> {
  const acc = await q("SELECT rider_id FROM offers WHERE order_id = $1 AND status = 'accepted'", [orderId]);
  await q("UPDATE offers SET status = 'expired' WHERE order_id = $1 AND status IN ('offered', 'accepted')", [orderId]);
  for (const row of acc.rows as Array<{ rider_id: string }>)
    await q("UPDATE rider_profiles SET status = 'online', updated_at = now() WHERE user_id = $1 AND status = 'busy'", [row.rider_id]);
}
let presenceSweepTimer: ReturnType<typeof setTimeout> | null = null;
async function sweepUnassignedDispatchable(): Promise<void> {
  try {
    await q(`UPDATE offers SET status = 'expired' WHERE status = 'offered' AND expires_at IS NOT NULL AND expires_at <= now()`);
    const r = await q(
      `SELECT id FROM orders WHERE status = ANY($1) AND NOT EXISTS (SELECT 1 FROM offers WHERE offers.order_id = orders.id AND (offers.status = 'accepted' OR (offers.status = 'offered' AND (offers.expires_at IS NULL OR offers.expires_at > now())))) ORDER BY created_at ASC LIMIT 20`,
      [DISPATCHABLE],
    );
    for (const row of r.rows as Array<{ id: string }>) await dispatchOrder(row.id);
  } catch (e) {
    app.log.error({ err: e }, 'presence sweep failed');
  }
}
function schedulePresenceSweep(): void {
  if (presenceSweepTimer) return; // collapse rapid presence flaps into one sweep
...
async function getOrderFull(orderId: string): Promise<Record<string, unknown> | null> {
  // Join buyer name/contact + counts here instead of letting clients fall back to placeholders.
  const order = await one<Record<string, unknown>>(`SELECT o.*, u.name AS buyer_name, u.email AS buyer_email, u.phone AS buyer_phone FROM orders o LEFT JOIN users u ON u.id = o.buyer_id WHERE o.id = $1`, [orderId]);
...
// Fail CLOSED: an unknown/missing owner never inherits the first owner's stores. Null -> 403.
async function resolveOwnerStoreIds(ownerEmail?: string): Promise<{ ownerId: string; storeIds: string[] } | null> {
  const owner = ownerEmail ? await one<{ id: string }>("SELECT id FROM users WHERE email = $1 AND role = 'store_owner'", [ownerEmail]) : undefined;
...
async function needOwner(req: FastifyRequest, reply: FastifyReply): Promise<{ ownerId: string; storeIds: string[] } | null> {
  const resolved = await resolveOwnerStoreIds((req.query as { ownerEmail?: string }).ownerEmail);
  if (!resolved) {
...
/** Any-stub-identity routes: 403 when no x-persona header was sent. */
const needPersona = (req: FastifyRequest, reply: FastifyReply): boolean => {
  if (!req.persona) {
    reply.code(403).send({ error: 'persona_required' });
    return false;
  }
  return true;
};
...
app.post('/api/orders', async (req, reply) => {
  ...
  // Order + items + payment land atomically — single transaction.
  ...
  try {
    ...
    for (const ln of lines)
      await client.query(`INSERT INTO order_items (order_id, item_id, name, unit_price, qty, options, line_total) VALUES ($1,$2,$3,$4,$5,$6,$7)`, [order.id, ln.itemId, ln.name, ln.unitPrice, ln.qty, JSON.stringify(ln.options), ln.lineTotal]);
    await client.query(`INSERT INTO payments (order_id, method, amount, status, ref_code) VALUES ($1,$2,$3,'succeeded',$4)`, [order.id, input.paymentMethod, total, `MOCK-${Date.now().toString(36).toUpperCase()}`]);
    await client.query('COMMIT');
  } catch (e) {
    try { await client.query('ROLLBACK'); } catch { /* report the original error */ }
    throw e;
  } finally {
    client.release();
  }
...
app.get<{ Params: { id: string } }>('/api/orders/:id', async (req, reply) => {
  if (!needPersona(req, reply) || !ok(req, reply, undefined, req.params.id, 'order id')) return;
...
app.post<{ Params: { id: string } }>('/api/orders/:id/cancel', async (req, reply) => {
  ...
  await releaseOrderAssignment(req.params.id); // free rider + expire live offers
...
app.post<{ Params: { id: string } }>('/api/orders/:id/review', async (req, reply) => {
  if (!ok(req, reply, ['buyer'], req.params.id, 'order id')) return;
  const parsed = reviewSchema.safeParse(req.body);
  if (!parsed.success) return badBody(reply, parsed.error.issues);
  const order = await one<{ id: string; buyer_id: string; store_id: string; status: string }>('SELECT id, buyer_id, store_id, status FROM orders WHERE id = $1', [req.params.id]);
  if (!order) return reply.code(404).send({ error: 'order_not_found' });
  // Reviews only for delivered orders, one per target.
  if (order.status !== 'delivered') return reply.code(409).send({ error: 'order_not_delivered', status: order.status });
  const dup = await q(`SELECT id FROM reviews WHERE order_id = $1 AND ((store_id IS NOT NULL AND rider_id IS NULL AND $2 = 'store') OR (rider_id IS NOT NULL AND store_id IS NULL AND $2 = 'rider')) LIMIT 1`, [order.id, parsed.data.target]);
...
app.get('/api/store/orders', async (req, reply) => {
  ...
  const owned = await needOwner(req, reply);
  if (!owned) return;
  const { storeIds } = owned;
  if (storeIds.length === 0) return { orders: [] };
  const qry = req.query as { status?: string };
  const params: unknown[] = [storeIds];
  let sql = `SELECT o.*, s.name AS store_name, u.name AS buyer_name, u.email AS buyer_email, u.phone AS buyer_phone, (SELECT count(*)::int FROM order_items WHERE order_id = o.id) AS items_count FROM orders o JOIN stores s ON s.id = o.store_id LEFT JOIN users u ON u.id = o.buyer_id WHERE o.store_id = ANY($1)`;
  if (qry.status) {
...
app.patch<{ Params: { id: string } }>('/api/store/orders/:id', async (req, reply) => {
  ...
  if (next === 'cancelled') {
    await refundPay(order.id);
    await releaseOrderAssignment(order.id); // free rider + expire live offers
  }
...
// The store client fetches single orders — owner-scoped, 404 outside own stores.
app.get<{ Params: { id: string } }>('/api/store/orders/:id', async (req, reply) => {
  if (!ok(req, reply, ['store_owner'], req.params.id, 'order id')) return;
  const owned = await needOwner(req, reply);
  if (!owned) return;
  const full = await getOrderFull(req.params.id);
  if (!full || !owned.storeIds.includes(full['store_id'] as string)) return reply.code(404).send({ error: 'order_not_found' });
  return { order: full };
});
app.get('/api/store/stats', async (req, reply) => {
  ...
  const owned = await needOwner(req, reply);
  if (!owned) return;
  const { ownerId, storeIds } = owned;
  if (storeIds.length === 0) return { ownerId, revenue_today: 0, active_count: 0, delivered_today: 0, store_count: 0 };
  const [rev, act, done] = await Promise.all([
    ...
    q(`SELECT count(*)::int AS delivered FROM orders WHERE store_id = ANY($1) AND status = 'delivered' AND created_at >= date_trunc('day', now())`, [storeIds]),
  ]);
  return { ownerId, store_count: storeIds.length, revenue_today: (rev.rows[0] as { revenue: number }).revenue, active_count: (act.rows[0] as { active: number }).active, delivered_today: (done.rows[0] as { delivered: number }).delivered };
});
...
app.post<{ Params: { id: string } }>('/api/offers/:id/accept', async (req, reply) => {
  ...
  // Bind the caller to the offer; hold a row lock across check+assign so concurrent accepts serialize.
  const callerId = await resolveRiderId(req.query);
  const client = await pool.connect();
  let orderId = '';
  try {
    await client.query('BEGIN');
    ...
    if (['delivered', 'cancelled', 'picked_up', 'delivering', 'rider_assigned'].includes(status)) {
      await client.query('ROLLBACK');
      return reply.code(409).send({ error: 'order_not_assignable', status });
    }
    orderId = offer.order_id;
    const nowIso = new Date().toISOString();
    await client.query("UPDATE offers SET status = 'accepted' WHERE id = $1", [offer.id]);
    await client.query("UPDATE offers SET status = 'expired' WHERE order_id = $1 AND status = 'offered' AND id <> $2", [offer.order_id, offer.id]);
    await client.query('UPDATE orders SET status = $2, timeline = timeline || $3::jsonb, updated_at = now() WHERE id = $1', [offer.order_id, 'rider_assigned', JSON.stringify({ at: nowIso, status: 'rider_assigned', note: `rider accepted offer ${offer.id}` })]);
  ...
  emit('offer.created', { id: req.params.id, status: 'accepted' }, orderId);
  emit('order.updated', { orderId, status: 'rider_assigned' }, orderId);
  return { order: await getOrderFull(orderId) };
});
app.post<{ Params: { id: string } }>('/api/offers/:id/decline', async (req, reply) => {
  if (!ok(req, reply, ['rider'], req.params.id, 'offer id')) return;
  const offer = await one<{ id: string; order_id: string; rider_id: string; status: string }>('SELECT * FROM offers WHERE id = $1', [req.params.id]);
  ...
  // Riders decline only their own offers (declining re-triggers dispatch).
  const callerId = await resolveRiderId(req.query);
  if (!callerId || callerId !== offer.rider_id) return reply.code(403).send({ error: 'offer_not_yours' });
...
app.post('/api/rider/presence', async (req, reply) => {
  ...
  if (parsed.data.status === 'online') schedulePresenceSweep(); // re-sweep stalled dispatchables
  return { rider: cur.rows[0] };
});
const pickupSchema = z.object({ pin: z.string().min(4).max(8) });
// Brute-force brake for the 4-digit pickup PIN (in-memory; 10 wrong tries lock the order 5 minutes).
...
app.post<{ Params: { id: string } }>('/api/orders/:id/pickup', async (req, reply) => {
  ...
  if ((order.pickup_pin ?? '') !== parsed.data.pin) {
    ...
    pinAttempts.set(order.id, { fails, lockedUntil: fails >= PIN_MAX_FAILS ? Date.now() + PIN_LOCK_MS : 0 });
...
app.post<{ Params: { id: string } }>('/api/tickets/:id/messages', async (req, reply) => {
  if (!needPersona(req, reply) || !ok(req, reply, undefined, req.params.id, 'ticket id')) return;
...
app.post<{ Params: { id: string } }>('/api/tickets/:id/resolve', async (req, reply) => {
  if (!ok(req, reply, ['operator', 'support'], req.params.id, 'ticket id')) return;
...
app.post<{ Params: { id: string } }>('/api/tickets/:id/refund', async (req, reply) => {
  ...
  // Refunds only make sense on terminal orders — a placed/preparing order keeps flowing after a refund.
  const st = await one<{ status: string }>('SELECT status FROM orders WHERE id = $1', [ticket.order_id]);
  if (!st) return reply.code(404).send({ error: 'order_not_found' });
  if (!['delivered', 'cancelled'].includes(st.status)) return reply.code(409).send({ error: 'order_not_refundable', status: st.status });
  await refundPay(ticket.order_id);
  await releaseOrderAssignment(ticket.order_id); // terminal here (guarded above), so freeing is always safe
  await appendTimeline(ticket.order_id, st.status, `refunded via support ticket ${ticket.id}`);
  emit('order.updated', { orderId: ticket.order_id, status: st.status }, ticket.order_id);
...
// Offer expiry derives from DB timestamps — recover anything a crash left 'offered', then keep sweeping.
...
io.on('connection', (socket) => {
  socket.on('join-order', (payload: unknown) => {
    ...
    if (orderId && z.string().uuid().safeParse(orderId).success) void socket.join(`order:${orderId}`);
  });
